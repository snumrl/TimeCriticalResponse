...
public class ActionSpatialMeasureModule extends Module{
	...
	protected void initializeImpl() {
		...
		String label = "jump_jog_ret_ue4_new_dy_ag";
...
//		saveFile = false;
		...
		for (int iter = 0; iter < sampleSize; iter++) {
			...
			double e = result.spatialError;
			h.addValue(e, prevGoal.actionType, goal.actionType);
			h.setLimitValue(e, prevGoal.actionType, goal.actionType);
...
//			h.setLimitValue((double)goal.timeLimit, prevGoal.actionType, goal.actionType);
		...
		String outputFolder = "output\\jumpAgility";
		if (saveFile) {
		...
		TimeBasedList<Point3d> rootTrajectory = new TimeBasedList<Point3d>();
		for (Motion m : mData.motionList) {
			Point3d p = new Point3d(MathUtil.getTranslation(m.root()));
			p.y = 0;
			rootTrajectory.add(p);
		}
		...
		getModule(ItemListModule.class).addSingleItem("rootTrajectory", rootTrajectory);
		...
		listMoudle.setItems(resultList, new ListListener<AgilityMeasureResult>(){
			...
			public String[] getColumnHeaders() {
				return new String[] { "prev action", "action", "Spatial Error", "tOver", "time limit", "c time", "fStart", "fEnd"};	
			}
	...
	public static void main(String[] args) {
		MainApplication.run(new ActionSpatialMeasureModule());
	}
}
