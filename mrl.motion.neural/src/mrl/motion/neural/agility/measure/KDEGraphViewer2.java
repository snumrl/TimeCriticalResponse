...
public class KDEGraphViewer2 extends ScalableCanvas2D{
	...
	public KDEGraphViewer2(Composite parent) {
		super(parent);
	...
	protected void drawContents(GC gc) {
		...
		if (drawSamplePoints) {
			for (int kIdx = kdeList.length-1; kIdx < kdeList.length; kIdx++) {
...
//					fillOval(gc, x-r, y-r, r*2, r*2);
		...
		for (int pIdx = 0; pIdx < pointCount; pIdx++) {
			...
			for (int kIdx = 0; kIdx < kdeList.length; kIdx++) {
				double p = kdeList[kIdx].getEstimatedWeight(new double[] { position });
...
//		}
		...
		drawYLabel(gc, "0.5", xRange, yRange, ratio(0.5, weightStat));
		drawYLabel(gc, "1", xRange, yRange, ratio(1, weightStat));
...
//		}
		...
		if (showLegend) {
			gc.setForeground(cm.getColor(0));
			drawString(gc, labels, xRange, 30, topRightLabelAlign, -1);
		}
