...
public abstract class PythonRuntimeController extends RuntimeController{
	...
	public PythonRuntimeController() {
		this(false);
	}
	public PythonRuntimeController(boolean useBall) {
		...
		app.dummyParent().getDisplay().timerExec(1, new Runnable() {
			...
			public void run() {
				...
				while (true){
					...
					if (frame == 0) startTime = System.currentTimeMillis();
					frame++;
					if (MotionDataConverter.useOrientation) {
						app.getModule(ItemListModule.class).addSingleItem(MOTION_LABEL, new MotionData(Utils.singleList(g.motion())));
					} else {
